...
export default function Home() {
  const hotProducts = products.filter(product => product.IS_HOT === true).slice(0, 4);
  const newProducts = products.filter(product => product.IS_NEW === true).slice(0, 6);
  return (
  ...
  <div className="w-full h-[700px] bg-gradient-to-b from-[#d0eaff] to-[#e0f2ff]">
    <div className="flex flex-col w-full h-full overflow-hidden rounded">
      <img src="/img/head.png" alt="" className="w-full h-full object-cover sm:h-[80%]" />
      <div className="p-6 pb-6 m-4 mx-auto -mt-16 space-y-6 lg:max-w-2xl sm:px-10 sm:mx-12 bg-white/90 backdrop-blur-md text-gray-900 rounded-lg shadow-xl">
        <div className="space-y-2">
          <a rel="noopener noreferrer" href="/" className="inline-block text-3xl font-bold">
  ...
  <div className="py-16 w-full bg-white">
    <div className="container px-6 mx-auto space-y-12">
      <div className="text-center space-y-2">
        <h2 className="text-[40px] font-extrabold text-gray-900">Các sản phẩm nổi bật</h2>
        <p className="text-lg text-gray-600">Những sản phẩm chất lượng theo thương hiệu</p>
      </div>
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
        {hotProducts.map((product) => (
          <div key={product.PRODUCT_ID} className="bg-white rounded-xl shadow-md overflow-hidden transition-transform hover:scale-[1.02]">
            <a rel="noopener noreferrer" href="#" aria-label={product.NAME}>
              <img alt={product.NAME} className="object-cover w-full h-52" src={product.IMAGE_URL} />
            </a>
            <div className="p-5 flex flex-col gap-2">
              <a rel="noopener noreferrer" href={`/category/${product.CATEGORY_ID.toLowerCase()}`} className="text-xs uppercase text-indigo-600 font-medium">
                {product.CATEGORY_ID}
              </a>
  ...
  <div className="py-16 w-full bg-[#e6f4ff]">
    <div className="container max-w-6xl px-6 mx-auto space-y-12">
      <div className="text-center space-y-2">
        <h2 className="text-[40px] font-extrabold text-gray-900">Những sản phẩm bán chạy</h2>
      </div>
      <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
        {newProducts.map((product) => (
          <div key={product.PRODUCT_ID} className="bg-white rounded-xl shadow-md overflow-hidden transition-transform hover:scale-[1.02]">
            <a rel="noopener noreferrer" href="#" className="block">
              <img alt="" className="object-cover w-full h-44" src={product.IMAGE_URL} />
  ...
  <div className="py-16 w-full bg-white">
    <div className="grid w-full grid-cols-1 px-6 mx-auto lg:px-8 md:grid-cols-2 md:divide-x">
      <div className="py-6 md:py-0 md:px-6">
        ...
        <p className="pt-2 pb-4 text-gray-600">Điền thông tin và chúng tôi sẽ liên hệ với bạn sớm nhất</p>
        <div className="space-y-4 text-gray-700">
          <p className="flex items-center">
            ...
            <span>Tòa T, Công viên phần mềm Quang Trung, Quận 12, TPHCM</span>
          </p>
          <p className="flex items-center">
      ...
      <form noValidate className="flex flex-col py-6 space-y-6 md:py-0 md:px-6">
        <label className="block">
          ...
          <input type="text" placeholder="Spring boot" className="block w-full px-4 py-3 rounded-lg border shadow-sm focus:ring focus:ring-indigo-300" />
        </label>
        <label className="block">
          <span className="mb-1">Địa chỉ Email</span>
          <input type="email" placeholder="java5@gmail.com" className="block w-full px-4 py-3 rounded-lg border shadow-sm focus:ring focus:ring-indigo-300" />
        </label>
        <label className="block">
          <span className="mb-1">Thắc mắc</span>
          <textarea rows="3" className="block w-full px-4 py-3 rounded-lg border shadow-sm focus:ring focus:ring-indigo-300"></textarea>
        </label>
        <button type="button" className="self-center px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow">Xác nhận</button>
      </form>
    </div>
  </div>
</div>
